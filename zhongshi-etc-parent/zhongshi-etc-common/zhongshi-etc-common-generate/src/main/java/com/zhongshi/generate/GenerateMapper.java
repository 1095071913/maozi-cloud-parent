		...
		for (EntityData entityData : entityDatas) {
			...
			if(!StringUtils.isEmpty(entityData.getForeignEntityOne())) {
				for(EntityData foreignEntity:entityData.getForeignEntityOne()) {
					...
					service.append("	public List<"+SQLType.initial(entityData.getModuleName())+"Do> select"+selectName+"(@Param(\""+entityData.getModuleName()+"\")"+SQLType.initial(entityData.getModuleName())+"Do "+entityData.getModuleName()+");\r\n\r\n");
				}
			}
