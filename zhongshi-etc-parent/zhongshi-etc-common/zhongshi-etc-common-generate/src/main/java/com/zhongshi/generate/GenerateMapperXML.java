...
public class GenerateMapperXML {
	...
	public static void generate(List<EntityData> entityDatas, String pash) throws Exception {
		...
		for (EntityData entityData : entityDatas) {
			...
			mapperXML.addAttribute("namespace", entityData.getPackageName()+".mapper."+SQLType.initial(entityData.getModuleName()) + "Mapper");
			/* mapper节点 End */
			...
			sqlEntityColumnXML.addAttribute("id", entityData.getModuleName()+"Column");
			String entityColumn="";
			for(String column:entityData.getField()) {
				if(entityColumn!="") {
					entityColumn+=",";
				}
				entityColumn+=entityData.getModuleName()+"."+column+" as "+entityData.getModuleName()+SQLType.initial(column);
			...
			List<String> foreignNames=new ArrayList<String>();
			if(entityData.getForeignEntityOne()!=null) {
				for(EntityData foreignEntity:entityData.getForeignEntityOne()) {
					foreignNames.add(foreignEntity.getModuleName());
				}
			}
			if(entityData.getForeignEntityMany()!=null) {
			...
			if(!StringUtils.isEmpty(entityData.getForeignEntityOne())) {
				for(EntityData foreignEntity:entityData.getForeignEntityOne()) {
					...
					for(String foreignColumn:foreignEntity.getField()) {
						...
						foreignEntityColumn+=foreignEntity.getModuleName()+"."+foreignColumn+" as "+SQLType.initial(foreignEntity.getModuleName());
					}
					...
					for(String entityColumn1:entityData.getField()) {
						String [] foreignKey = entityColumn1.split("Id"); 
						if(
								
								!(foreignKey.length==1&&foreignNames.contains(foreignKey[0]))
								&&   
								!(foreignKey.length==2&& foreignNames.contains(foreignKey[0]) && foreignKey[1].equals("s"))
					...
					for(String entityColumn1:entityData.getField()) {
						...
						if(entityColumn1.equals("id")) {
							...
							columnEntity.addAttribute("column", entityData.getModuleName()+SQLType.initial(entityColumn1));
						}
					...
					for(String entityForeignColumn:foreignEntity.getField()) {
						...
						if(entityForeignColumn.equals("id")) {
							...
							columnEntity.addAttribute("column", foreignEntity.getModuleName()+SQLType.initial(entityForeignColumn));
						}
			...
			if(!StringUtils.isEmpty(entityData.getForeignEntityMany())) {
				...
				for(EntityData foreignEntity:entityData.getForeignEntityMany()) {
					...
					for(String foreignColumn:foreignEntity.getField()) {
						...
						foreignEntityColumn+=foreignEntity.getModuleName()+"."+foreignColumn+" as "+foreignEntity.getModuleName()+SQLType.initial(foreignColumn);
					}
					sqlForeignEntityColumnXML.addText(foreignEntityColumn);
					mapperXML.add(sqlForeignEntityColumnXML);
					...
					/* 一对一连接查询 Begin */
					selectOneToOne.addText(" from "+entityData.getModuleName());
					selectOneToOne.addText(" inner join " + foreignEntity.getModuleName() + " on "+entityData.getModuleName()+"."+foreignEntity.getModuleName()+"Ids = " + foreignEntity.getModuleName()+".id");
					/* 一对一连接查询 End */
			...
			doc.add(mapperXML);
			/* 创建XML Begin */
			createFile(entityData,pash,doc);
			/* 创建XML End */
		}
	...
	private static void createFile(EntityData entityData,String pash,Document doc) throws Exception {
		String [] fileURLs=((entityData.getPackageName())+".mapper.").split("\\.");
		...
		XMLWriter writer = new XMLWriter(new FileOutputStream(pash + "\\" + SQLType.initial(entityData.getModuleName()) + "Mapper" + ".xml"), format);
		writer.write(doc);
		writer.flush();
		writer.close();
	}
