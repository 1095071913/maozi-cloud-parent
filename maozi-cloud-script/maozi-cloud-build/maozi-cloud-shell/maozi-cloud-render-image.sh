...
# ------------------------------------------------------------
# 被 maozi-cloud-build-jar-utils.sh / maozi-cloud-build-all-distributed-force.sh
# 通过 source 调用, 暴露 render_and_build_image 函数.
# 对应 bat 版: maozi-cloud-bat/maozi-cloud-render-image.bat + .ps1
# ------------------------------------------------------------
# 设计要点:
...
render_and_build_image() {
    ...
    local image_root="$current_directory/../../maozi-cloud-image"
...
if [ "${BASH_SOURCE[0]}" = "$0" ]; then
    ...
    if [ -z "$service_name" ]; then
        ...
        echo "Services are defined in maozi-cloud-image/maozi-cloud-services.json"
        exit 1
    fi
    ...
    image_root="$current_directory/../../maozi-cloud-image"
