...
public class RestConfigServiceImpl extends ConfigServiceImpl implements RestConfigService {
	 ...
	 * 分页查询全局配置列表
	 *
	 * @param pageParam 分页查询参数，包含分页信息和配置列表查询条件（类型、名称）
	 * @return 分页查询结果，包含配置列表数据
	 */
	@Override
	public AbstractBaseResult<PageResult<ConfigListVo>> restList(PageParam<ConfigListParam> pageParam) {
		return ResultUtil.success(listRelation(pageParam, ConfigListVo.class));
	}

	/**
	 * 新增全局配置
	 *
	 * @param param 配置新增参数，包含配置名称、类型、配置值
	 * @return 新增配置的 ID
	 */
	@Override
	 ...
	 *
	 * @param name 配置名称
	 * @return 该配置名称对应的配置信息，包含配置 ID、名称、别名与配置值
	 */
	@Override
	 ...
	 */
	@Override
	public AbstractBaseResult<ConfigInfoVo> restGet(Long id) {
		return ResultUtil.success(getByIdThrowErrorRelation(id, ConfigInfoVo.class));
	}
	 ...
	 *
	 * @param id    需要更新的配置 ID
	 * @param param 配置更新参数，包含需要修改的名称、类型、配置值
	 * @return 操作结果
	 */
	@Override
